...
from utils import _is_pil_image,_is_tensor_image,_is_numpy_image
...
class Resize(object):
    ...
    def __init__(self, size, interpolation='lanczos'):
    ...
    def __call__(self, img_arr):
        ...
        if issubdtype(ts, np.signedinteger):
            ...
            size = tuple((np.array(img.size)*percent).astype(int))
            #print(size)
        elif issubdtype(ts, np.floating):
            size = tuple((np.array(img.size)*self.size).astype(int))
        else:
            size = (self.size[1], self.size[0])
        func = {'nearest': 0, 'lanczos': 1, 'bilinear': 2, 'bicubic': 3}
        resample=func[self.interpolation]

        if img_arr.ndim == 3:

            return np.array(img.resize(size, resample=resample))
        elif img_arr.ndim == 2:
            return np.array(img.resize(size, resample=resample))
        else:
            RuntimeError('img should be ndarray with 2 or 3 dimensions. Got {}'.format(img_arr.ndim))
